...
<head>
    ...
    <title>🔥 Firuman Live Sales Dashboard - Billy Bob's 8 Stores</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .store-name { font-size: 1.4em; font-weight: bold; margin-bottom: 12px; color: #fff; }
        .store-sales { font-size: 2.2em; color: #FF4500; font-weight: bold; }
        .store-orders { font-size: 1em; color: #ccc; margin-top: 8px; }
        .footer { 
            text-align: center; 
            margin-top: 40px; 
            padding-top: 20px; 
...
<body>
    <div class="header">
        ...
        <h2 style="color: #ccc; font-size: 1.4em;">Billy Bob's 8 Store Network</h2>
        <p id="current-date">{{ now()->format('l, F j, Y') }}</p>
        <p id="last-updated">Loading...</p>
    </div>

    <div id="dashboard-content">
        <div class="loading">📊 Loading Billy Bob's live sales data...</div>
    </div>

    <div class="footer">
        <p>🔥 <strong>Firuman POS</strong> | Billy Bob's Real-time Dashboard</p>
        <p>Auto-refreshes every 30 seconds | Perfect for smaller operations</p>
    </div>
    ...
    <script>
        ...
        async function loadDashboard() {
            ...
            try {
                const response = await fetch('/api/messages/live-sales-dashboard/8', {
                    headers: {
                        'Accept': 'application/json'
                    }
                });

                if (!response.ok) throw new Error('Failed to fetch Billy Bob\'s dashboard data');
                ...
                content.innerHTML = `
                    <div class="error">
                        <h3>❌ Billy Bob's dashboard is having trouble</h3>
                        <p>${error.message}</p>
